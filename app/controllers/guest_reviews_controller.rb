class GuestReviewsController < ApplicationController
  ...
  def create
    # Step 1: Check if the reservation exist (craft_id, host_id, host_id)
    ...
    @reservation = Reservation.where(
                    ...
                    craft_id: guest_review_params[:craft_id]
                   ).first

    if !@reservation.nil? && @reservation.craft.user.id == guest_review_params[:host_id].to_i
  ...
  private
    def guest_review_params
      params.require(:guest_review).permit(:comment, :star, :craft_id, :reservation_id, :host_id)
    end
end
